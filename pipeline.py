...
def extract_documents(raw_text: str) -> dict:
    ...
    m = re.search(
        r"Между\s+([^()«»\d\n]{3,80}?)\s*\(\s*(?:Истец[,\s]*)?Заимодавец\s*\)\s*и\s+([^()«»\d\n]{3,80}?)\s*\(\s*(?:Ответчик[,\s]*)?За[ёе]мщик",
...
def extract_from_certificate(raw_text: str) -> dict:
    ...
    # LOANDATE_START — "DD.MM.YYYY заключили сделку займа ..."
    m = re.search(r"(\d{2}\.\d{2}\.\d{4})\s+заключили\s+сделку", raw_text)
    if m:
        out["{{LOANDATE_START}}"] = m.group(1)

    # LOAN_AMOUNT — "Сумма займа\n<число>" (формат Свидетельства, без ", бел. руб.")
...
def extract_strict_markers(raw_text: str) -> dict:
    ...
    pair_re = re.compile(r"\bс\s*(" + _DATE + r")\s+по\s*(" + _DATE + r")", re.IGNORECASE)
